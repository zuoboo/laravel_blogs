            ...
            <div class="py-4 bg-white rounded">
                <form action="{{ route('admin.menus.update', ['menu' => $menu->id]) }}" method="POST"
                    ...
                    <div class="pt-4 px-6">
                        ...
                        <!-- ▲▲▲▲エラーメッセージ▲▲▲▲　-->
                        <div class="mb-6">
                            <label class="block text-sm font-medium mb-2" for="name">商品名</label>
                            <input id="name" class="block w-full px-4 py-3 mb-2 text-sm bg-white border rounded"
                                type="text" name="name" value="{{ old('name', $menu->name) }}">
                        </div>

                        <div class="mb-6">
                            <label class="block text-sm font-medium mb-2" for="image">画像</label>
                            <div class="flex items-end">
                                <img id="previewImage" src="{{ asset('storage/' . $menu->image) }}"
                                    data-noimage="{{ asset('storage/' . $menu->image) }}" alt=""
                        ...
                        <div class="mb-6">
                            ...
                            <input id="price" class="block w-full px-4 py-3 mb-2 text-sm bg-white border rounded"
                                type="number" name="price" value="{{ old('price', $menu->price) }}">
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="introduction" class="block w-full px-4 py-3 mb-2 text-sm bg-white border rounded" name="introduction"
                                rows="5">{{ old('introduction', $menu->introduction) }}</textarea>
                        </div>
                    </div>
                </form>
            </div>
